...
X.renderer2D.prototype.onWindowLevel_ = function(event) {
   ...
   // console.info('onWindowLevelExecute' + event);
  // grab the current volume
  if(this.ctrlDown()){
        var _volume = this._topLevelObjects[0];
...
X.renderer2D.prototype.__defineSetter__('orientation', function(orientation) {
  ...
  var _volume = this._topLevelObjects[0];
  //this._camera.setOrientation(orientation);//this is done on the renderer camera is not ready here
...
X.renderer2D.prototype.update_ = function(object) {
  ...
  if (!(object instanceof X.volume)) {
	  ...
	  this.m_dimensions = object._dimensions;
          this.m_spacing = object._RASSpacing;
          this.m_origin = object._RASOrigin;
	  ...
	  if (this._orientationIndex == 0) {
	    ...
	    _currentSlice = object['indexZ'];
          ...
          if(object._children[this._orientationIndex]._children[_currentSlice] === undefined){
              return;
          }
...
X.renderer2D.prototype.render_ = function(picking, invoked) {
  ...
  if(this._slices[parseInt(_currentSlice, 10)] === undefined){
      return;
  }
  ...
  this._context.setTransform(_normalizedScale, 0, 0, _normalizedScale, _width / 2, _height / 2);
  ...
  var _redraw_required = (this._currentSlice != _currentSlice ||
      ...
      .compare(_labelmapShowOnlyColor, this._labelmapShowOnlyColor, 0, 0, 4)) || this._volumeLabelsChanged === 1);
  ...
  var _offset_x = -_sliceWidth * this._sliceWidthSpacing / 2.0 + _dx;
  var _offset_y = -_sliceHeight * this._sliceHeightSpacing / 2.0 + _dy;
  
  this._offset_x = -_sliceWidth * this._sliceWidthSpacing / 2.0 + _dx;
  this._offset_y = -_sliceHeight * this._sliceHeightSpacing / 2.0 + _dy;
 ...
 * @return {?string} The orientation of this renderer.
 */
X.renderer2D.prototype.__defineGetter__('offset', function() {

  return [this._offset_x, this._offset_y];
 ...
 * @return {?string} The orientation of this renderer.
 */
X.renderer2D.prototype.__defineGetter__('sliceSize', function() {

  return [this._sliceWidth * this._sliceWidthSpacing, this._sliceHeight * this._sliceHeightSpacing];
 ...
 * @return {?string} The orientation of this renderer.
 */
X.renderer2D.prototype.__defineGetter__('dimensions', function() {

  return this.m_dimensions;

});

X.renderer2D.prototype.__defineGetter__('context', function() {
 ...
 * @inheritDoc
 */
X.renderer2D.prototype.setAnnotationTable = function(annotations) { 
...
X.renderer2D.prototype.xy2ijk = function(x, y) {
    ...
    //window.console['debug']("start xyijk");
    
    var _volume = this._topLevelObjects[0];
    var _view = this._camera._view;
     var _currentSlice = null;
    //window.console['debug']("view = "  + _view);
    ...
    var scalesizex = _sliceWidth * this._sliceWidthSpacing;
    var scalesizey = _sliceHeight * this._sliceHeightSpacing;
  ...
  // .. this includes zoom
    ...
    var pointx = (x - this._width / 2)/_normalizedScale;
    var pointy = (y - this._height / 2)/_normalizedScale;
    ...
    /*console.log("x, y = "  + x +", " + y);
    console.log("cx, cy = "  + pointx +", " + pointy);
    console.log("ofx, ofy = "  + _offset_x +", " + _offset_y); */
    
    //window.console['debug']("ofx, ofy = "  + _offset_x +", " + _offset_y);
    
    /*this._context.beginPath();
    this._context.moveTo(_offset_x, _offset_y);
    this._context.lineTo(pointx, pointy);
    this._context.strokeStyle = "#00FF00";
    this._context.stroke();
    this._context.closePath();*/
    
    if(this.isLeft([pointx, pointy], [_offset_x, _offset_y], [_offset_x, _offset_y + scalesizey])
       && this.isLeft([pointx, pointy], [_offset_x, _offset_y + scalesizey], [_offset_x + scalesizex, _offset_y + scalesizey])
       && this.isLeft([pointx, pointy], [_offset_x + scalesizex, _offset_y + scalesizey], [_offset_x + scalesizex, _offset_y])
       && this.isLeft([pointx, pointy], [_offset_x + scalesizex, _offset_y], [_offset_x, _offset_y])){
   
       //window.console['debug']("inside = "  + pointx +", " + pointy);
       
       
       
       var spacing = this.m_spacing;
       var dim = this.m_dimensions;
       var origin = this.m_origin;
       
       var _ijk = goog.vec.Vec4.createFloat32FromValues(0, 0, 0, 1);
       
       
              
       //window.console['debug']("slide info = "  + this._sliceWidth +", " + this._sliceWidthSpacing + ", " + this._sliceHeight +", " + this._sliceHeightSpacing);
       //window.console['debug']("vol info = "  + dim +", " + spacing + ", " + origin);
       
       var coordx = (pointx - _offset_x)/(this._sliceWidth * this._sliceWidthSpacing);
       var coordy = (pointy - _offset_y)/(this._sliceHeight* this._sliceHeightSpacing);
       ...
       if(this._orientation === "X"){
            ...
            _ijk[0] = this._currentSlice;
            _ijk[1] = (coordy)*dim[1];
            
            //if(this._radiological){
                _ijk[2] = dim[2] - (coordx)*dim[2];
            //}else{
            //    _ijk[2] = (coordx)*dim[2];
            //}
            ...
            //console.log("insideA "  + _ijk[0] + ", " + _ijk[1] + ", " + _ijk[2]);
            ...
            //coordx = (pointx - _offset_x)/(this._sliceWidth * this._sliceWidthSpacing);
            //coordy = (pointy - _offset_y)/(this._sliceHeight* this._sliceHeightSpacing);
            
            //if(this._radiological){
                _ijk[0] = dim[0] - (coordx)*dim[0];
            //}else{
            //    _ijk[0] = (coordx)*dim[0];
            //}            
            //_ijk[0] = (coordx)*dim[0]*spacing[0];
            //_ijk[1] = (coordy)*dim[1]*spacing[1];
            _ijk[1] = this._currentSlice;
            _ijk[2] = dim[2] - (coordy)*dim[2];
            
            this._orientationColors[0] = 'red';
            this._orientationColors[1] = 'blue';
            
        }/*else if(this._orientation === "Z"){				    	
            //console.log("insideB "  + _ijk[0] + ", " + _ijk[1] + ", " + _ijk[2]);
            ...
            //if(this._radiological){
                _ijk[0] = dim[0] - (coordx)*dim[0] - 1;
            //}else{
            //    _ijk[0] = (coordx)*dim[0];
            //}
            _ijk[1] = dim[1] - (coordy)*dim[1] - 1;
            _ijk[2] = this._currentSlice;
        ...
        /*this._context.beginPath();
        this._context.moveTo(_offset_x, _offset_y);
        this._context.lineTo(_offset_x, _offset_y + this._sliceHeight* this._sliceHeightSpacing);
        ...
        this._context.beginPath();
        this._context.moveTo(_offset_x, _offset_y);
        this._context.lineTo(_offset_x + this._sliceWidth * this._sliceWidthSpacing, _offset_y);
...
X.renderer2D.prototype.ctrlDown = function() {
	return this._interactor._ctrlKeyDown;
};
...
goog.exportSymbol('X.renderer2D.prototype.ijk2xy',
    X.renderer2D.prototype.ijk2xy);
